...
new class extends Component {
    ...
    {
        $emparejamientos = Emparejamiento::with([
            'jugadorBlancas.equipo',
            'jugadorNegras.equipo'
        ])
            ->where('ronda_id', $rondaId)
                ...
                foreach ($this->torneo->rondas as $ronda) {
                    $emp = Emparejamiento::where('ronda_id', $ronda->id)
                        ->where(function ($q) use ($jugador) {
                            $q->where('blancas_id', $jugador->id)
                                ->orWhere('negras_id', $jugador->id);
                        })
                        ->first();

                    $puntos = 0;
                    if ($emp) {
                        $puntos = $emp->blancas_id == $jugador->id
                            ? $emp->puntos_blancas
                            : $emp->puntos_negras;
                    }
    ...
    public function getGraficaEquiposProperty()
{
    ...
    $data = $this->tablaGeneral->values()->sortByDesc(function($row) {
...
<div class="min-h-screen bg-slate-50 font-sans text-slate-900 pb-12">
    ...
    <header class="bg-[#c5a059] py-10 mb-10 shadow-lg relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-4 relative z-10">
            <h1
                class="text-4xl md:text-5xl font-black text-white text-center uppercase tracking-tighter drop-shadow-md">
                        ...
                        <tbody>
                            @foreach($this->tablaGeneral->values() as $index => $row)
                                @php
                                    ...
                                    // Definimos los colores fijos para las medallas
                                    $medallaStyle = match ($pos) {
                                        1 => 'background-color: rgba(255, 215, 0, 0.2) !important; border-left: 6px solid #FFD700;', // Oro
                                        2 => 'background-color: rgba(192, 192, 192, 0.2) !important; border-left: 6px solid #C0C0C0;', // Plata
                                        3 => 'background-color: rgba(205, 127, 50, 0.2) !important; border-left: 6px solid #CD7F32;',  // Bronce
                                        default => ''
                                    };
                                @endphp
                                ...
                                <tr style="{{ $medallaStyle }}" class="border-b transition hover:bg-slate-50">
                                    ...
                                    {{-- ... resto de tus columnas (Rondas, Global, Individual) ... --}}
                                    @foreach($row['rondas'] as $r)
                                        <td class="p-3">
                                            @if($r['global'] !== null)
                                                <div class="font-bold text-slate-800">{{ $r['global'] }}</div>
                                                <div class="text-[10px] text-slate-400">({{ $r['individual'] }})</div>
                                            @else
                        ...
                        <tbody>
                            @foreach($this->tablaIndividual->values() as $index => $row)
                                @php
                                    ...
                                    $medallaStyle = match ($pos) {
                                        1 => 'background-color: rgba(255, 215, 0, 0.2) !important; border-left: 6px solid #FFD700;',
                                        2 => 'background-color: rgba(192, 192, 192, 0.2) !important; border-left: 6px solid #C0C0C0;',
                                        3 => 'background-color: rgba(205, 127, 50, 0.2) !important; border-left: 6px solid #CD7F32;',
                                        default => ''
                                    };
                                @endphp
                    ...
                    @foreach($emparejamientos->groupBy('estacion') as $estacion => $partidas)
                        ...
                        <div class="mb-10 last:mb-0">
                            <div class="flex items-center justify-between mb-4 bg-slate-900 p-3 rounded-lg shadow-md">
                                <span class="text-white font-bold px-3 py-1 bg-[#c5a059] rounded text-sm uppercase">Estación
                                    {{ $estacion }}</span>
                                ...
                                @foreach($partidas as $emp)
                                    <div
                                        ...
                                        <div class="md:col-span-1 flex flex-col items-center justify-center gap-2">
                                            ...
                                            {{-- SELECT RESULTADO --}}
                                            <select wire:change="guardarResultado({{ $emp->id }}, $event.target.value)"
                                                class="border border-slate-300 rounded px-2 py-1 text-xs font-bold bg-white focus:ring-2 focus:ring-[#c5a059]">
                                                <option value="">-</option>
                                                <option value="1-0" {{ $emp->resultado == '1-0' ? 'selected' : '' }}>1-0</option>
                                                <option value="0-1" {{ $emp->resultado == '0-1' ? 'selected' : '' }}>0-1</option>
                                                <option value="1-1" {{ $emp->resultado == '1-1' ? 'selected' : '' }}>1-1</option>
                                            </select>
                            ...
                            @php
                                $equiposIds = $partidas->map(function ($p) {
                                    return [
                                        $p->jugadorBlancas->equipo_id,
                                        $p->jugadorNegras->equipo_id
                                    ];
                                })->flatten()->unique();
                                            ...
                                            @foreach($resultados as $res)
                                                <div
                                                    ...
                                                    <div class="flex gap-3">
                                                        {{-- Puntos Individuales --}}
                                                        <div class="flex flex-col items-end border-r border-slate-100 pr-3">
                                                            <span class="text-[9px] text-slate-400 font-bold uppercase">Indiv.</span>
                                                            <span
                                                        ...
                                                        <div class="flex flex-col items-end">
                                                            <span class="text-[9px] text-[#c5a059] font-black uppercase">Global</span>
